...
const videoData: Record<Category, { src: string; title: string }[]> = {
  socialmedia: [
...
export default function Showcase() {
  ...
  // Wrap position to stay within [0, -singleSetWidth] for infinite loop
  ...
  // Pause/resume auto-scroll on hover/drag
  useEffect(() => {
  ...
  // Drag — document-level events with infinite wrapping
  useEffect(() => {
  ...
  return (
    <section className="relative w-full overflow-x-clip">
      ...
      >
        <div
          ref={trackRef}
          className="flex gap-0 h-full absolute left-0 top-0"
        >
